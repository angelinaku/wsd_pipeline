...
from datasets.dataset_collator import WSDDataset, WSDCollator
...
class DatamoduleWSD(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        # Loading file with mapping of words to their ids (if needed)
        if self.conf.data.word_to_idx_name:
            with open(f'{self.conf.data.main_folder}{self.conf.data.word_to_idx_name}', 'r', encoding='utf-8') as f:
                self.word_to_idx = json.load(f)
        else:
            self.word_to_idx = {}

        self.wsd_collator = WSDCollator(percentile=self.conf.training.collator.percent,
        ...
        if self.conf.data.vectorization is True:
            self.embedder = Embedder(word_to_idx=self.word_to_idx, embeddings_path=self.conf.data.embeddings_path,
                                     embeddings_type=self.conf.data.embeddings_type,
